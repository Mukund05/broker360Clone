...
const Agencies = () => {
  ...
  const handleCollab = async (id) => {
    ...
    try {
      const response = await Api.collaborations(data);
      console.log(response);
      if (response.success) {
        // Re-fetch user data after successful collaboration invite
        const updatedResponse = await Api.getUsers();
        if (updatedResponse.success) setUser(updatedResponse.data);
      }
    } catch (error) {
      ...
      <div className="p-6 bg-[#eff6ff]">
        ...
        <div className="flex flex-col gap-8 items-center">
          {user &&
            user.map((agent) => (
              <div
                className="bg-white p-2 flex flex-col gap-2 w-[95%] md:w-3/4 py-4 hover:cursor-pointer"
                key={agent.id}
              >
                ...
                <div
                  className="flex justify-between gap-3 px-4 flex-col sm:flex-row"
                  onClick={() => navigate(`/agency/${agent.id}`)}
                >
                  <div className="w-1/5 h-fit justify-center flex items-center">
                    <img
                      src={
                        agent.profile_url
                          ? `${import.meta.env.VITE_BASE_URL}${agent.profile_url}`
                          : agency1
                      }
                    ...
                    <div className="flex flex-col gap-2 w-1/3 items-end">
                      ...
                      <div className="flex gap-1">
                        <span className="text-[16px] ">
                          {agent.office ? agent.office : "N/A"}
                        </span>
                      </div>
                      <div className="flex gap-1">
                        <span className="text-[16px] ">
                          {agent.number ? agent.number : "N/A"}
                        </span>
                      </div>
                      <div className="flex gap-1">
                        <span className="text-[16px] ">
                ...
                <div className="flex justify-between flex-col sm:flex-row items-center gap-3 w-full">
                  ...
                  <div className="w-1/3 flex justify-center sm:justify-end">
                    {agent.collaboration ? (
                      <button
                        className="p-2 border border-[#6e6e70] flex justify-between gap-3 items-center bg-white rounded-md h-fit w-fit"
