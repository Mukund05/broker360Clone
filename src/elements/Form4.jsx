...
const Form4 = ({ propertyData, onFormDataChange }) => {
  const [active, setActive] = useState(0);

  const handleValueChange = (e) => {
    return () => {
      setActive(e);
      onFormDataChange({ ...propertyData, share_commission: e });
    }; 
  }
      ...
      <div className="flex justify-start  items-start sm:items-center gap-x-6 flex-col sm:flex-row">
        ...
        <div className="p-2 px-4 flex justify-between w-full sm:w-2/3 border border-[#8692A6] rounded-md">
          <textarea
            ...
            rows={6}
            col={20}
            name="condition_sharing"
            value={propertyData.condition_sharing}
            onChange={(e) =>
              onFormDataChange({
                ...propertyData,
                condition_sharing: e.target.value,
              })
            }
          />
        </div>
